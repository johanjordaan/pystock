...
# config.probability
def random(config,day_end_data):
    ...
    for tkr in day_end_data['tkr'].unique():
        ...
        entries = tkr_day_end_data.sample(frac=config["probability"])
...
# config.window
def daily_range_breakout(config,day_end_data):
    ...
    for tkr in day_end_data['tkr'].unique():
        tkr_day_end_data = day_end_data[day_end_data['tkr'] == tkr]
        tkr_day_end_data_ma = tkr_day_end_data.rolling(window=config["window"], min_periods=1).mean()
